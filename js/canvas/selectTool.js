...
export class SelectTool {
  constructor(canvas) {
    ...
    this.isSelecting = false; // 标记是否正在框选
  ...
  handleMouseDown(event) {
    ...
    if (hitResult) {
      ...
      if (newItemHit) {
        ...
        this.dragStartPoint = event.point;
        this.selectionRectangle = this.highlightUI.showSelectionRect(
          event.point
        );
      }
    }
  ...
  handleMouseDrag(event) {
    if (this.isSelecting && this.selectionRectangle) {
      const rect = new paper.Rectangle(this.dragStartPoint, event.point);
      this.selectionRectangle = this.highlightUI.updateSelectionRect(
  ...
  handleMouseUp(event) {
    if (this.isSelecting && this.selectionRectangle) {
      const bounds = this.selectionRectangle.bounds;
      this.selectionRectangle.remove();
      this.selectionRectangle = null;
      this.isSelecting = false;

      const selected = paper.project.activeLayer.children.filter(
        (item) => bounds.intersects(item.bounds) && !item.data?.isUIElement // 使用统一的标记来过滤UI元素
      );

      if (!event.modifiers.shift) {
        this.clearSelection();
      }
      selected.forEach((item) => this.selectItem(item));
